...
/*  GNU ed - The GNU line editor.
    ...
    Copyright (C) 2020 Raman Gopalan <ramangopalan@gmail.com>
    Modified for Elle, the toy, self hosted computer.
...
int read_file( const char * const filename, const int addr )
  ...
  if( *filename == '!' ) fp = fopen( filename + 1, "r" );
  ...
  if( *filename == '!' ) ret = fclose( fp ); else ret = fclose( fp );
  if( size < 0 ) return -1;
  if( ret != 0 )
    {
    show_strerror( filename, errno );
...
int write_file( const char * const filename, const char * const mode,
  ...
  if( *filename == '!' ) fp = fopen( filename + 1, "w" );
