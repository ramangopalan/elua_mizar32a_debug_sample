...
/* Raman: Open edtmp, modify, close and remove after file I/O */
#define ED_TEMP_FILE            "/mmc/edtmp"
...
FILE * shellfp = 0;         /* scratch shell file pointer */
...
// Raman: GNU ed: scratch file(s) remove
...
bool close_shellbuf( void )
  ...
  if( shellfp )
    ...
    if( fclose( shellfp ) != 0 )
      {
      show_strerror( 0, errno );
      set_error_msg( "Cannot close shell file" );
      return false;
      }
    shellfp = 0;
    }
...
bool init_buffers( void )
  ...
  /* Read stdin one character at a time to avoid i/o contention
     ...
     hello, world
     EOF */
  /* Raman: setvbuf( stdin, 0, _IONBF, 0 );
   * I noticed when I entered Ed for a quick edit session and went back
   * to the main shell. I wasn't able to backspace.
   */
...
bool open_sbuf( void )
  ...
  sfp = fopen( ED_TEMP_FILE, "wb+" ); /* Raman: tmpfile(); */
  if( !sfp )
    {
    show_strerror( 0, errno );
...
/* open scratch file for shell commands */
bool open_shellbuf( void )
  {
  shellfp = fopen( ED_TEMP_FILE_SHELL, "wb+" ); /* Raman: tmpfile(); */
  if( !sfp )
    {
    show_strerror( 0, errno );
    set_error_msg( "Cannot open shell file" );
    return false;
    }
  return true;
  }
